...
const ClosetManagementTable = () => {
  ...
  const fetchClosetItems = useCallback(
    async ({ pageNumber = page, size = pageSize, filters: f = filters } = {}) => {
      setLoading(true);
      setError(null);
      ...
      console.log("[Closet] fetching page", pageNumber, "pageSize", size, "filters", f);
      ...
      try {
        ...
        console.warn("[Closet] fetch error, using SAMPLE_DATA fallback:", err.message);
      ...
      <div className="relative inline-block" ref={ref}>
        ...
        {open && (
          <div className="absolute right-0 mt-2 w-40 bg-white border rounded-md shadow z-50">
            <button onClick={handleView} className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50">View</button>
            <button onClick={handleEdit} className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50">Edit</button>
            <button onClick={handleArchiveAction} className="w-full text-left px-3 py-2 text-sm text-red-600 hover:bg-gray-50">Archive</button>
          </div>
        )}
      </div>
    );
    ...
    <div className="bg-white rounded-md shadow-sm p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="text-xs text-gray-500">Showing {items.length} of {total} results</div>
        </div>
      ...
      <div className="md:hidden space-y-3">
        {loading ? (
          ...
          items.map((it) => (
            <div key={it.id} className="p-3 bg-gray-50 rounded-md flex items-start justify-between">
              <div>
                <div className="text-sm font-semibold text-gray-800">{it.title}</div>
                <div className="text-xs text-gray-500">{it.category} • {it.uploadedBy}</div>
                <div className="mt-1 text-xs">
                  {it.tags && it.tags.map((t, i) => (
                    <span key={i} className="inline-block bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full mr-1 text-xs">{t}</span>
                  ))}
                </div>
              </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-4 py-6 text-center text-gray-600">No items found</td></tr>
              ) : (
                items.map((it) => (
                  <tr key={it.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      {it.tags && it.tags.map((t,i) => <span key={i} className="inline-block bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full mr-1 text-xs">{t}</span>)}
                    </td>
                    <td className="px-4 py-3">{it.uploadedBy}</td>
                    <td className="px-4 py-3"><span className={`px-2 py-1 rounded-full text-xs ${it.status === 'Uploaded' ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'}`}>{it.status}</span></td>
                    <td className="px-4 py-3">{it.dateAdded}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <ActionMenu item={it} />
                      </div>
      ...
      <div className="mt-4 flex items-center justify-end text-sm">
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={page === 1}
            className={`px-3 py-1 border rounded ${page === 1 ? 'text-gray-300 border-gray-200' : 'text-gray-600 hover:bg-gray-50'}`}
          >
            Prev
          </button>
            ...
            if (totalPages <= maxButtons) {
              return Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                <button
                  ...
                  className={`w-10 h-8 rounded-md text-sm ${p === page ? 'bg-[#bfc986] text-gray-900 font-semibold' : 'border text-gray-600 hover:bg-gray-50'}`}
                >
                  {p}
                </button>
              ));
          ...
          <button
            ...
            className={`px-3 py-1 border rounded ${page === totalPages ? 'text-gray-300 border-gray-200' : 'text-gray-600 hover:bg-gray-50'}`}
          >
            Next
          </button>
        </div>
