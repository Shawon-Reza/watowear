 ...
 */
export default function UsersHistoryManagement({
	users = [],
	...
	const filtered = useMemo(() => {
		...
		if (tab !== "All")
			list = list.filter(
				(u) => (u.status || "").toLowerCase() === tab.toLowerCase()
			);
					...
					<tbody>
						{pageItems.map((u) => (
							<tr key={u.id} className="border-t">
								...
								<td className="py-4 px-4">
									<span
										className={`inline-flex items-center px-2 py-1 rounded text-xs ${statusClass(
											u.status
										)}`}
									>
										{u.status}
