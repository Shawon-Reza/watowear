...
const AI_Performence = () => {
  ...
  const stats = {
    suggestionsAccuracy: { value: 92.7, change: 2.3, percent: 92.7, color: "#6366F1" },
    userAcceptance: { value: 78.4, change: 4.1, percent: 78.4, color: "#22C55E" },
    avgResponse: { value: "1.2s", change: null, percent: 40, color: "#3B82F6" },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        <div className="p-4 rounded-xl border-l-4 border-l-[#6A6D57] bg-white shadow-sm hover:shadow transition-shadow duration-150">
          <h3 className="text-sm text-[#6B7280]">Suggestions Accuracy</h3>
          <div className="flex items-center justify-between py-3">
            <h1 className="font-extrabold text-2xl">{stats.suggestionsAccuracy.value}%</h1>
            <div className="p-1 rounded-md bg-gray-50">
              <BrainCircuit size={20} className="text-[#6366F1]" />
            </div>
          </div>
          <p className="text-xs text-[#6B7280]">
            <span className="text-green-500">+{stats.suggestionsAccuracy.change}%</span> vs last month
          </p>
          <div className="w-full bg-gray-100 rounded-full h-1.5 mt-2">
            <div className="h-1.5 rounded-full" style={{ width: `${stats.suggestionsAccuracy.percent}%`, background: stats.suggestionsAccuracy.color }} />
          </div>
        </div>

        {/* User Acceptance Rate */}
        <div className="p-4 rounded-xl border-l-4 border-l-[#6A6D57] bg-white shadow-sm hover:shadow transition-shadow duration-150">
          <h3 className="text-sm text-[#6B7280]">User Acceptance Rate</h3>
          <div className="flex items-center justify-between py-3">
            <h1 className="font-extrabold text-2xl">{stats.userAcceptance.value}%</h1>
            <div className="p-1 rounded-md bg-gray-50">
              <CircleCheckBig size={20} className="text-[#22C55E]" />
            </div>
          </div>
          <p className="text-xs text-[#6B7280]">
            <span className="text-green-500">+{stats.userAcceptance.change}%</span> vs last month
          </p>
          <div className="w-full bg-gray-100 rounded-full h-1.5 mt-2">
            <div className="h-1.5 rounded-full" style={{ width: `${stats.userAcceptance.percent}%`, background: stats.userAcceptance.color }} />
          </div>
        </div>

        {/* Avg Response Time */}
        <div className="p-4 rounded-xl border-l-4 border-l-[#6A6D57] bg-white shadow-sm hover:shadow transition-shadow duration-150">
          <h3 className="text-sm text-[#6B7280]">Avg. Response Time</h3>
          <div className="flex items-center justify-between py-3">
            <h1 className="font-extrabold text-2xl">{stats.avgResponse.value}</h1>
            <div className="p-1 rounded-md bg-gray-50">
              <Clock4 size={20} className="text-[#3B82F6]" />
            </div>
          </div>
          <p className="text-xs text-[#6B7280]">{stats.avgResponse.change !== null ? `${stats.avgResponse.change}% vs last month` : 'N/A'}</p>
          <div className="w-full bg-gray-100 rounded-full h-1.5 mt-2">
            <div className="h-1.5 rounded-full" style={{ width: `${stats.avgResponse.percent}%`, background: stats.avgResponse.color }} />
          </div>
        </div>

        {/* Total Suggestions */}
        <div className="p-4 rounded-xl border-l-4 border-l-[#6A6D57] bg-white shadow-sm hover:shadow transition-shadow duration-150">
          <h3 className="text-sm text-[#6B7280]">Daily Suggestions</h3>
          <div className="flex items-center justify-between py-3">
            <h1 className="font-extrabold text-2xl">{stats.totalSuggestions.value.toLocaleString()}</h1>
            <div className="p-1 rounded-md bg-gray-50">
              <TfiStatsUp size={20} className="text-[#A855F7]" />
            </div>
          </div>
          <div className="w-full bg-gray-100 rounded-full h-1.5 mt-2">
            <div className="h-1.5 rounded-full" style={{ width: `${stats.totalSuggestions.percent}%`, background: stats.totalSuggestions.color }} />
          </div>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-4 sm:p-6 shadow drop-shadow-md rounded-[10px]">
            {isEditing ? (
              ...
              <p className="text-[#747474] leading-relaxed text-sm">{text}</p>
            )}
          </div>
